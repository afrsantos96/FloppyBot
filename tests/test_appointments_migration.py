...
def _old_schema_svs_conn():
    ...
    conn.execute("INSERT INTO appointments VALUES (1, 'Construction Day', '10:00', 5)")
    conn.execute("INSERT INTO appointments VALUES (2, 'Research Day', '14:00', 5)")
    conn.commit()
    return conn
...
def test_migration_preserves_existing_rows():
    ...
    assert rows == [
        (1, None, "Construction Day", "10:00", 5),
        (2, None, "Research Day", "14:00", 5),
    ]
...
def test_migrated_table_allows_manual_name_row_alongside_fid_rows():
    ...
    conn.execute(
        ...
        "VALUES (NULL, 'Guest Governor', 'Troops Training Day', '09:00', NULL)"
    )
    conn.commit()

    row = conn.execute(
        "SELECT manual_name FROM appointments WHERE appointment_type='Troops Training Day' AND time='09:00'"
    ).fetchone()
...
def test_migrated_table_rejects_double_booking_same_slot():
    ...
    conn.execute(
        "INSERT INTO appointments (fid, manual_name, appointment_type, time, alliance) "
        "VALUES (NULL, 'Guest A', 'Troops Training Day', '09:00', NULL)"
    )
    conn.commit()
    ...
    try:
        conn.execute(
            "INSERT INTO appointments (fid, manual_name, appointment_type, time, alliance) "
            "VALUES (NULL, 'Guest B', 'Troops Training Day', '09:00', NULL)"
        )
        conn.commit()
...
def test_migrated_table_allows_two_different_manual_names_on_different_slots():
    ...
    conn.execute(
        "INSERT INTO appointments (fid, manual_name, appointment_type, time, alliance) "
        "VALUES (NULL, 'Guest B', 'Troops Training Day', '09:30', NULL)"
    )
    conn.commit()

    count = conn.execute(
        "SELECT COUNT(*) FROM appointments WHERE appointment_type='Troops Training Day'"
    ).fetchone()[0]
    assert count == 2
...
def _old_schema_archive_conn():
    ...
    conn.execute("INSERT INTO minister_archive_appointments VALUES (1, 1, 'Construction Day', '10:00', 5, 'Alice')")
    conn.commit()
    return conn
...
def test_archive_migration_adds_manual_name_and_relaxes_fid():
    ...
    conn.execute(
        ...
        "VALUES (1, NULL, 'Guest Governor', 'Research Day', '11:00', NULL, 'Guest Governor')"
    )
    conn.commit()
...
def test_archive_migration_preserves_existing_row():
    ...
    assert row == (1, 1, None, "Construction Day", "10:00", 5, "Alice")
